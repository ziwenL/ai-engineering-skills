...
- 与 iOS 相关的团队规范、命名规则、依赖管理方式和相似实现
...
1. 阅读项目入口文件和 `skills/common/`，确认当前任务是创建 iOS 新项目骨架，而不是继续修改已有 iOS 工程。
...
5. 输出推荐默认方案、备选方案与切换条件，并说明为什么默认采用单工程 + SwiftUI + 最小职责分层，而不是默认切到复杂多 target 或 package 结构。
6. 定义最小工程结构，包括应用入口、主题或样式入口、首页壳、状态边界、服务边界、导航占位和资源目录。
7. 输出首批应创建的目录、文件和初始化步骤，并说明每一项存在的原因。首批文件通常至少应覆盖工程入口、`App` 启动文件、首页视图、基础状态容器、服务协议或网络占位、资源目录和必要配置文件。
8. 明确 view、state、service、navigation 的最小边界，避免在 scaffold 阶段把业务细节提前固化。
9. 单独说明与 `ios-development` 的衔接方式，包括目录如何继续扩展、状态占位如何转成真实实现、服务占位如何替换为正式接入，以及何时再评估是否升级到更复杂的工程结构。
10. 明确本阶段不应提前做的事，例如过早拆 `workspace`、补齐完整业务流、引入复杂依赖管理或提前做跨端实现细节绑定。
11. 输出验证方式、未验证项和风险点。
