...
- Android 相关团队规范、包结构偏好、构建约束和相似实现
...
4. 如果默认方案采用 Jetpack Compose，把 Compose Coding Convention 一并作为骨架默认约束写清楚：
...
5. 明确第一阶段的目标是最小可运行壳子，而不是一次性铺开完整业务实现。
6. 输出推荐默认方案、备选方案与切换条件，并说明为什么默认采用单 `app` 模块加按功能包组织的目录策略。
7. 定义最小工程结构，包括应用入口、主题、基础页面壳、网络占位、数据边界和路由占位。
8. 输出首批应创建的目录、模块、文件和初始化步骤，并说明每一项存在的原因。
9. 明确本阶段不应提前做的事，例如过早拆分模块、补齐完整业务流、引入复杂发布或 CI 配置。
10. 说明与 `android-development` 的衔接方式。
11. 输出验证方式、未验证项和风险点。
...
- 如果默认技术栈选择 Jetpack Compose，必须把 `@Preview`、`ViewModel`、`fake/mock`、`private` 和 `network/database` 这些 Preview 约束写入脚手架决策或初始模板说明。
...
要求：
- 只创建最小可运行壳子。
- 如果默认使用 Compose，把 Compose Coding Convention 一起落到脚手架约定里。
- 不要提前展开完整业务逻辑。
```
