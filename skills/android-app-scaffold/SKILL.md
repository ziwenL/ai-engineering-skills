...
- 与 Android 相关的团队规范、包结构偏好、构建约束和相似实现
- `scaffold-checklist.md`
- `default-stack.md`

## 工作流程

1. 阅读项目入口文件和 `skills/common/`，确认当前任务是创建 Android 新项目骨架，而不是继续修改已有 Android 工程。
...
5. 输出推荐默认方案、备选方案与切换条件，并说明为什么默认采用单 `app` 模块加按功能包组织的目录策略，而不是默认切到复杂多模块或横切分层。
6. 定义最小工程结构，包括应用入口、主题、基础页面壳、网络占位、数据边界和路由占位。
7. 输出首批应创建的目录、模块、文件和初始化步骤，并说明每一项存在的原因。首批文件通常至少应覆盖 `settings.gradle.kts`、根 `build.gradle.kts`、`app/build.gradle.kts`、`AndroidManifest.xml`、应用入口、主页面壳和必要的占位层。
8. 明确本阶段不应提前做的事，例如过早拆分模块、补齐完整业务流、引入复杂发布或 CI 配置。
9. 单独说明与 `android-development` 的衔接方式，包括包结构如何继续扩展、状态边界如何从占位层转为真实实现、网络或数据占位如何替换成正式接入。
10. 输出验证方式、未验证项和风险点。
